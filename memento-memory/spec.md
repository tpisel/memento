...
There is **no `write` verb** (removed by ADR-0031). Agents author note bodies with their **native file tools** (Claude: Write / Edit / MultiEdit / Bash; codex: `apply_patch` / shell). Mode is enforced at the tool boundary by a **PreToolUse `check-write` hook** that classifies the intended write, reads old-bytes + ratification from disk and git, and returns an allow/deny verdict *before* the bytes land — append-only keeps old content as a prefix, read-only requires new == old, living always allows. The selector is the byte-level prefix invariant rather than an explicit operation flag: ADR-0015 keeps absent or `append-only` frontmatter conservative by default, while ADR-0017 makes unratified rewrites legal without making replacement implicit. This is what prompt-instruction cannot make reliable on its own: an agent can rationalise past a written rule, but a ratified `read-only` doc is denied at the gate — to change an accepted decision after first commit the agent **must** author a superseding record, not quietly rewrite history. ADR-0017 makes the commit-as-review-boundary claim load-bearing across all three modes: unratified notes accept appends and overwrites regardless of declared mode; ratified notes obey the mode table above. Enforcement is an explicit **cooperative-agent guardrail** (it runs in user-editable client config and is fail-open on hook absence); the integrity floor is the committed diff plus `compile`/pre-commit drift and MODE VIOLATION detection (ADR-0031), not the hook alone.

### When the agent should write of its own accord (default triggers)

Trigger-shaped, not discretion-shaped — discretionary "write down useful things" yields noise-or-nothing; specific triggers yield signal. These live in a tool-read writing guide so they are tunable per-project without a recompile, and are **read at write-time**. ADR-0010 pins the file at `_memento/writing.md`: free-form prose, optional, project-curated. When present, the writing convention is surfaced through `memento orient` (ADR-0010; it carries a `when_to_read:` of *"before authoring or editing durable memento memory"*). The tool does not reinforce the instruction at write-time and does not detect "read this session" (statelessness, see ADR-0010). Greenfield `init` scaffolds a minimal writing.md; adoption never clobbers.
...
**In-session consistency via auto-compile (ADR-0022, re-homed by ADR-0031).** A native write to a vault note fires a **PostToolUse `compile` hook** that rebuilds `.memento/manifest.json` and `_memento/brief.md`, so a subsequent `memento read @N` or `memento brief` resolves against the new state without an explicit recompile step. The recompile is re-homed from the (removed) write verb's tail onto the hook; it also carries the drift / MODE VIOLATION detection (ADR-0031). The cost is bounded — `BenchmarkCompile500Docs` is ~18ms on Apple M2 Max — and idempotent with the pre-commit hook. Recompile is now per-write rather than per-verb-call; debounce / incremental compile is the follow-up if the hot path bites.
...
# note bodies are written with native tools, gated by the PreToolUse check-write hook (ADR-0031) — there is no write verb
memento write-mode <key> <append-only|living|read-only> [--justification <reason>]  # durably change a note's mode; loosening requires --justification (ADR-0031)
memento unlock <key> --justification <reason>  # temporary single-key read-only exception until next commit (ADR-0031)
```
...
### Write enforcement contract (ADR-0031, supersedes the ADR-0022 write-verb contract)

There is no write verb to emit a stderr contract. Body writes go through native tools, and two hooks carry what the verb used to:

- **PreToolUse `check-write`** (hook plumbing, hidden from help) consumes the raw PreToolUse payload on stdin, resolves the `file_path` (or parses the Bash command / `apply_patch` envelope) to a vault-relative key, and emits the harness verdict JSON on stdout: `{hookSpecificOutput:{permissionDecision, permissionDecisionReason}}`. The `reason_code` is deliberately **not** on the wire (codex's strict schema rejects unknown keys and would fall open); it rides the gitignored `.memento/` decision log instead. The dumb-pipe bash wrapper fails **closed** (deny) on any internal error for an in-vault target.
- **PostToolUse `compile`** rebuilds the manifest/brief and raises a loud `DRIFT ALARM` (gated bytes ≠ landed bytes) or `MODE VIOLATION` (on-disk diff breaks a ratified note's mode, ungated) on stderr. The git pre-commit hook runs the same audit as the catch-all at the ratification boundary; `MEMENTO_STRICT_COMMIT` flips it from detection to commit-blocking.

Vault-relative key rules are unchanged; a path that resolves to a git-ignored, operational (`_memento/`), or non-`.md` target is denied `unwritable_path`. Mode changes route exclusively through `write-mode`; loosening justifications append to the decision log (ADR-0031, 2026-06-28 addendum — no commit trailers).
...
- **Post-write manifest/brief refresh** — resolved by ADR-0022, re-homed by ADR-0031 onto the PostToolUse `compile` hook (the write verb it tailed is removed); no opt-out flag; pre-commit hook remains idempotent on top.
