...
- **Compile-error handling.** A write that succeeds followed by a compile that fails is an awkward state — the body is on disk, the manifest is not refreshed. The chosen behaviour is: write returns success, recompile error surfaces as a stderr warning, exit code reflects the recompile failure. Open to revision if this confuses agents.
