...
- `memento doctor [--session]` — run the check DAG over a memento installation across two axes: **liveness** (is write enforcement actually on here — gate wired, binary reachable, no legacy broad-deny, live-fire self-test) and **hygiene** (is the install well-formed — manifest freshness, config validity, ignore stanzas). Diagnose-only. Exit `0` = no gating finding, `1` = a gating finding, `2` = usage error; a hygiene error or a no-vault run exits nonzero without meaning enforcement is specifically OFF. Warnings exit `0` by default and gate only under `MEMENTO_DOCTOR_STRICT`. `--session` defers the expensive checks (manifest recompile, live-fire) off the SessionStart hot path. See `memento doctor --help` and ADR-0032 for the full check catalog.
