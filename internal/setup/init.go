...
func claudeOrientHookScriptContents(repoRoot string) string {
	return strings.Join([]string{
		...
		"# checks (manifest-fresh recompile, live-fire probe) off the hot path; a binary too",
		"# old for the flag has no enforcement marker, folding into the same unavailable branch.",
		"doctor_output=\"$(cd \"$repo_root\" && memento doctor --session 2>&1)\"",
		"doctor_status=$?",
